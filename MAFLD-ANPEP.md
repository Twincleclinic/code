...
dirs <- dirs[file.exists(file.path(dirs,"matrix.mtx"))]
objects <- lapply(dirs,function(x){obj<-Read10X(data.dir=x);CreateSeuratObject(counts=obj,project=basename(x),min.cells=3,min.features=200)})
...
ggsave("subcluster_umap.pdf",width=6,height=5)
...
ggsave("dotplot.pdf",width=8,height=4)
FeaturePlot(subset_obj,features=markers_select,cols=c("lightblue","red"))
ggsave("featureplot.pdf",width=8,height=6)
...
cds <- setOrderingFilter(cds,disp_table$gene_id)
cds <- reduceDimension(cds,max_components=2,method="DDRTree")
cds <- orderCells(cds)
plot_cell_trajectory(cds,color_by="Pseudotime")
ggsave("pseudotime.pdf",width=5,height=5)
expr_anpep <- as.numeric(sce@assays$RNA@data["ANPEP",])
sce$ANPEP_group <- ifelse(expr_anpep>median(expr_anpep),"High","Low")
DimPlot(sce,reduction="umap",group.by="ANPEP_group",cols=c("#999999","#E41A1C"))
ggsave("group_umap.pdf",width=6,height=5)
...
ggsave("KEGG_low.pdf",width=6,height=5)
...
ggsave("KEGG_high.pdf",width=6,height=5)
plot_cell_trajectory(cds,color_by="celltype")
ggsave("pseudotime_celltype.pdf",width=5,height=5)
plot_genes_in_pseudotime(cds[rowData(cds)$gene_short_name=="ANPEP",])
ggsave("ANPEP_pseudotime.pdf",width=5,height=4)
DimPlot(sce,reduction="umap",group.by="ANPEP_group",split.by="celltype",cols=c("#999999","#E41A1C"))
ggsave("split_celltype.pdf",width=6,height=5)
DimPlot(sce,reduction="umap",group.by="ANPEP_group",split.by="orig.ident",cols=c("#999999","#E41A1C"))
ggsave("split_sample.pdf",width=6,height=5)
